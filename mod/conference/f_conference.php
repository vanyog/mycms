...
//           if(isset($_GET['rename']) && ($_GET['rename']=='on')) $d = conference_rename_files($d);
        ...
        if ( $team || $allowtoshow ) if($d['approved_a']) conference_add_auth($auth, $d['authors']);
...
//     if ( $team ) $rz .= "<p>$zip_command </p>\n";
     if (!$current || ("$today">="$day_a_approve") || $team || $preview ) $rz .= $sr;
     $tc += $c;
     $docs += $doc;
     $pdfs += $pdf;
   }
...
if ( $team || $allowtoshow ){
     $rz = "<p>".count($auth).
           " authors, $tc abstracts, $docs doc, $pdfs pdf, $rc ready. Sort: $olink ".
           " Secret link: <a href=\"".
...
// Пренасочване поради допусната грешка в изпратени писма
if($page_id==67){
  $h = str_replace('pid=67&', 'pid=92&', $_SERVER['REQUEST_URI']).'#main';
  header("Location: $h");
  die;
}
// Данни за доклада
