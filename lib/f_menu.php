...
$pp = stored_value('menu_popup');  // Дали да се показва изскачащо меню
foreach($d as $m){
  $lnn = 1*$m['link'];
  $ln = $m['link']; 
...
//
// Тази функция определяне дали точка от менюто, сочеща към страница 
...
//
// Изскачащо подменю на меню с номер $i, към точка със запис $m
//
